...
var EditingController = modules.ViewController.inherit(function () {
  ...
  return {
    ...
    _editRowFromOptionChangedCore: function _editRowFromOptionChangedCore(rowIndices, rowIndex, preventRendering) {
      ...
      data: {
        ...
        _handleDataSourceChange(args) {
          ...
          if (Array.isArray(dataSource) && changes.length) {
            ...
            var newChanges = changes.filter(change => {
            ...
            var isEditNewItem = newChanges.find(change => change.type === 'insert' && equalByValue(editRowKey, change.key));
